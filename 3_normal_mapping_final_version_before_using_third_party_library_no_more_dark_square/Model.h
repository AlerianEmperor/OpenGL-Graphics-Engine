...
GLuint LoadTexture(const string& filepath, bool bump_map = false, float bump_strength = 1.0f)
{
	...
	if (data)
	{
		...
		if (bump_map)
		{
			...
			{
				for (int i = 0; i < w * h; ++i)
					gray_scale.push_back(data[i] / 255.0f);
			}
			else
			{
				int count = 0;
				for (int i = 0; i < w; ++i)
				{
					for (int j = 0; j < h; ++j)
					{
						...
						float r = data[n * index] / 255.0f;
						float g = data[n * index + 1] / 255.0f;
						float b = data[n * index + 2] / 255.0f;

						//vec3(0.2126, 0.7152, 0.0722)
						//gray_scale.push_back(b);//(r + g + b) * 0.333f);
						//gray_scale.push_back((r + g + b) * 0.333f);

						gray_scale.push_back(b);//r * 0.2126 + g * 0.7152 + b * 0.0722);
			...
			//if (is_normal_map)
			{
				for (int i = 0; i < w * h; ++i)
				{
					float r = data[n * i] / 255.0f;
					float g = data[n * i + 1] / 255.0f;
					float b = data[n * i + 2] / 255.0f;

					real_texture.push_back(r);
					real_texture.push_back(g);
					real_texture.push_back(b);
				}
			}

			//convert bump map to normal map
			//else
			//	real_texture = Bump_Map_To_Normal_Map(w, h, n, bump_strength, gray_scale);

			glBindTexture(GL_TEXTURE_2D, textureID);
			glTexImage2D(GL_TEXTURE_2D, 0, format, w, h, 0, format, GL_FLOAT, &real_texture[0]);
...
GLuint Load_Bump_Map(const string& filepath, float& bump_strength)
{
	...
	glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA, w, h, 0, GL_RGBA, GL_FLOAT, flat_out_texture);
...
struct Vertex
{
	...
	Vertex(vec3 v_, vec3 vn_, vec2 vt_, vec3 tangent_, vec3 biTangent_) : v(v_), vn(vn_), vt(vt_) , tangent(tangent_), biTangent(biTangent_) {}
};
...
struct Mesh
{
	...
	void setup_mesh()
	{
		...
		for (int j = 0; j < vertices.size(); ++j)
			vertex.emplace_back(Vertex(vertices[j], normals[j], texcoords[j], tangents[j], biTangents[j]));
		...
		//{
			...
			glVertexAttribPointer(4, 3, GL_FLOAT, GL_FALSE, sizeof(Vertex), (void*)44);//(3 * sizeof(float)));//12
		//}

		vertices.swap(vector<vec3>());
		normals.swap(vector<vec3>());
		texcoords.swap(vector<vec2>());
		tangents.swap(vector<vec3>());
		biTangents.swap(vector<vec3>());
...
struct Model
{
	...
	Model(const string& filepath)
	{
		...
		for (int i = 0; i < meshes.size(); ++i)
		{
			...
			meshes[i].normal_ind = mats[i].Texture_Normal_Id;
	...
	void Read_Material(const string& filepath)
	{
		...
		while (f2.getline(line, 256))
		{
			...
			else if (strncmp(t, "map_bump", 8) == 0 || strncmp(t, "map_Bump", 8) == 0)
			{
				...
				{
					//cout << "Read Bump!\n";
					//GLuint TextureId = LoadTexture(path, true, 10.0f);
	...
	void Read_Model(const string& filepath)
	{
		...
		vec3 ce = (min_vector + max_vector) * 0.5f;
		...
		for (int m = 0; m < mats.size(); ++m)
		{
			for (int i = 0; i < meshes[m].index.size(); i += 3)
			{
				...
				vec3 t0 = tangent_map[i0];
				vec3 t1 = tangent_map[i1];
				vec3 t2 = tangent_map[i2];

				vec3 bt0 = biTangent_map[i0];
				vec3 bt1 = biTangent_map[i1];
				vec3 bt2 = biTangent_map[i2];

				meshes[m].tangents.emplace_back(t0);
				meshes[m].tangents.emplace_back(t1);
				meshes[m].tangents.emplace_back(t2);

				meshes[m].biTangents.emplace_back(bt0);
				meshes[m].biTangents.emplace_back(bt1);
				meshes[m].biTangents.emplace_back(bt2);
			}
		}
