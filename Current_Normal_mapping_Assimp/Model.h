...
private:
	...
	Mesh processMesh(aiMesh *mesh, const aiScene *scene)
	{
		...
		// 4. height maps
		std::vector<Texture> normalMaps = loadMaterialTextures(material, aiTextureType_HEIGHT, "Normal_Texture");//"Height_Texture");
		textures.insert(textures.end(), normalMaps.begin(), normalMaps.end());

		// return a mesh object created from the extracted mesh data
		return Mesh(vertices, indices, textures, Kd);
	}
	...
	vector<Texture> loadMaterialTextures(aiMaterial *mat, aiTextureType type, string typeName)
	{
		...
		for (unsigned int i = 0; i < mat->GetTextureCount(type); i++)
		{
			...
			{   // if texture hasn't been loaded already, load it
				...
				bool bump = type == aiTextureType_HEIGHT;
				if (bump)
					cout << str.C_Str() << "\n";
				Texture texture;
				texture.id = TextureFromFile(str.C_Str(), this->directory, gammaCorrection, bump, 10.0f);
...
unsigned int TextureFromFile(const char *path, const string &directory, bool gamma, bool bump, float bump_strength)
{
	...
	if (data)
	{
		...
		/*else
		{
			...
			{
				flat_out_texture[3 * i] = 0;// (result[i].x);// / 255.0f;
				flat_out_texture[3 * i + 1] = 0;// (result[i].y);// / 255.0f;
				flat_out_texture[3 * i + 2] = 0;//(result[i].z);// / 255.0f;
			}
			vector<vec3>().swap(result);

			glBindTexture(GL_TEXTURE_2D, textureID);
			glTexImage2D(GL_TEXTURE_2D, 0, GL_RGB, w, h, 0, GL_RGB, GL_UNSIGNED_BYTE, flat_out_texture);
		}*/
