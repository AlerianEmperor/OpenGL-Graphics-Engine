...
static vector<vec3> Bump_Map_To_Normal_Map_Correct(const int& w, const int& h, const int& n, const float& bump_strength, const vector<vec3>& grayscale)
{
	...
	for (int i = 0; i < h; ++i)
	{
		for (int j = 0; j < w; ++j)
		{
			...
			/*const float tl = abs(grayscale[top_left]);
			const float t = abs(grayscale[top]);
			const float tr = abs(grayscale[top_right]);

			const float l = abs(grayscale[left]);
			const float c = abs(grayscale[center]);
			const float r = abs(grayscale[right]);

			const float bl = abs(grayscale[bottom_left]);
			const float b = abs(grayscale[bottom]);
			const float br = abs(grayscale[bottom_right]);*/
			...
			//const float dy = (tl + 2.0 * t + tr) - (bl + 2.0 * b + br);
			const float dy = (bl + 2.0 * b + br) - (tl + 2.0 * t + tr);
			const float dz = 100;//(bump_strength);
...
GLuint Load_Bump_Map(const string& filepath, float& bump_strength)
{
	...
	//if (normal_map)
	{
		...
		{
			flat_out_texture[3 * i] = (gray_scale[i].x);
			flat_out_texture[3 * i + 1] = (gray_scale[i].y);
			flat_out_texture[3 * i + 2] = (gray_scale[i].z);
		}
	}
	/*else
	{
		...
		for (int i = 0; i < result.size(); ++i)
		{
			...
			flat_out_texture[3 * i] = (result[i].x);
			flat_out_texture[3 * i + 1] = (result[i].y);
			flat_out_texture[3 * i + 2] = (result[i].z);
		}
	}*/
		...
		while (f.getline(line, 256))
		{
			...
			else if (strncmp(t, "map_bump", 8) == 0 || strncmp(t, "map_Bump", 8) == 0)
			{
				...
				if (texture_map.find(name) == texture_map.end())
				{
					//cout << "Read Bump!\n";
					GLuint TextureId = LoadTexture(path, true, 0.5f);
					//GLuint TextureId = Load_Bump_Map(path, g_bump_strength);
